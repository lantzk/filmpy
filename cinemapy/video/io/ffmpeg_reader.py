...
class FFmpegInfosParser:
    ...
    def parse_video_stream_data(self, line):
        ...
        if self.fps_source == "fps":
            ...
            raise ValueError(
                f"fps source '{self.fps_source}' not supported parsing the video '{self.filename}'"
            )
    ...
    def parse_tbr(self, line):
        ...
        tbr = float(s_tbr[:-1]) * 1000 if s_tbr[-1] == "k" else float(s_tbr)
        return tbr
...
def ffmpeg_parse_infos(
    ...
    try:
        ...
        raise OSError(f"Error passing `ffmpeg -i` command output:\n\n{infos}") from exc
